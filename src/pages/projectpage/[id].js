...
export default function Blog() {
  ...
  const articoli = async () => {
    ...
    setPost({ ...project, gallery: updatedGallery });
  };
  ...
  const settings = {
    ...
    beforeChange: () => {
      ...
      videos.forEach((video) => {
        video.pause();
        video.currentTime = 0; // opzionale, riavvia anche il video
      });
    },
    ...
    <div>
      ...
      <div className="contentArea">
        ...
        <div className="sliderMobile">
          {post?.gallery?.map((item, i) => (
            item.isVideo ? (
              <video key={i} className="galleryImg" src={item.url} controls width="100%" />
            ) : (
              <img key={i} className="galleryImg" src={item.url} alt="Gallery" loading="eager" />
            )
          ))}
        </div>
